...
import {
  MissedDcTransfer,
  Vessel2vesselModel,
} from "./layout/reports/dpr/sov/models/Transfers/vessel2vessel/Vessel2vessel";
import { V2vCtvActivity } from "./layout/reports/dpr/sov/models/Transfers/vessel2vessel/V2vCtvActivity";
import {
  ForecastOperation,
  ForecastResponseObject,
  MetoceanProvider,
} from "./layout/forecast/models/forecast-response.model";
...
export class CommonService {
  ...
  get(url: string): Observable<any> {
    return this.http
      .get(environment.DB_IP + url, httpOptions)
      .pipe(catchError(this.getServerErrorMessage));
  }
  post(url: string, data: any): Observable<any> {
    return this.http
      .post(environment.DB_IP + url, data, httpOptions)
      .pipe(catchError(this.getServerErrorMessage));
  }
  put(url: string, data: any): Observable<any> {
    return this.http
      .put(environment.DB_IP + url, data, httpOptions)
      .pipe(catchError(this.getServerErrorMessage));
  }
  ...
  getVesselsForClientByUser(
    username: string
  ): Observable<UsermanagementVesselModel[]> {
    return this.get("/api/getVesselsForClientByUser/" + username);
  }
  getVesselNameAndIDById(info: {
    vessel_ids: number[];
  }): Observable<{ vessel_id: number; nicename: string }[]> {
  ...
  // ============= CTV DPR =============
  getLatestGeneral(): Observable<
    { _id: number; date: number; vesselname: string }[]
  > {
  ...
  getTransfersForVesselByRangeForCTV(
    vessel: StatsRangeRequest
  ): Observable<any[]> {
  ...
  getVessel2vesselsForSov(
    mmsi: number,
    date: number
  ): Observable<Vessel2vesselModel[]> {
  ...
  getPlatformLocations(src_name: string) {
    ...
    return this.post("/api/getPlatformLocations/", {
      Name: "Northsea_offshore_oilgas_platform_coordinates",
    });
  }
  getHarbourLocations() {
    return this.get("/api/getHarbourLocations/");
  }
  getForecastProjectLocations(): Observable<
    { lon: number; lat: number; nicename: string; id: number }[]
  > {
  ...
  // ============= Wavedata =============
  getFieldsWithWaveSourcesByCompany(): Observable<
    { _id: string; site: string; name: string }[]
  > {
  ...
  getWavedataForDay(request: {
    date: number;
    site: string;
  }): Observable<Array<{ wavedata: RawWaveData }>> {
  ...
  getTurbineTransfersForVesselByRangeForSOV(
    vessel: StatsRangeRequest
  ): Observable<any[]> {
    return this.post("/api/getTurbineTransfersForVesselByRangeForSOV/", vessel);
  }
  getPlatformTransfersForVesselByRangeForSOV(
    vessel: StatsRangeRequest
  ): Observable<any[]> {
    return this.post(
      "/api/getPlatformTransfersForVesselByRangeForSOV/",
      vessel
    );
  }
  getGeneralForVesselByRangeForSOV(
    vessel: StatsRangeRequest
  ): Observable<any[]> {
    return this.post("/api/getGeneralForVesselByRangeForSOV/", vessel);
  }
  getTransitsForVesselByRangeForSOV(
    vessel: StatsRangeRequest
  ): Observable<any[]> {
  ...
  getForecastProjectByName(
    project_name: string
  ): Observable<ForecastOperation[]> {
  ...
  getForecastWorkabilityForProject(
    project_id: number
  ): Observable<ForecastResponseObject> {
  ...
  getForecastWeatherForResponse(
    id: number
  ): Observable<{ weather: RawWaveData; spectrum: any }> {
    return this.post("/api/mo4light/weather", {
      response_id: id,
    });
  }
  saveForecastProjectSettings(
    project: ForecastOperation
  ): Observable<{ data: string }> {
...
interface CtvDprAccessHoursWeatherModel {
  accessDayType: string;
