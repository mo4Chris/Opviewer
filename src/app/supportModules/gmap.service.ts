...
export class GmapService {
  ...
  private addPlatformToLayer(layer: MapZoomLayer, platform: OffshorePlatformWithData) {
    ...
    if (this.settings.dpr_map_drawExclusionZone) {
      new google.maps.Circle({
        center: markerPosition,
        radius: this.settings.dpr_map_drawExclusionZone,
        map: layer.map,
        fillOpacity: 0,
        strokeColor: platform.isVisited ? 'rgb(53,255,68)' : 'black',
        strokeWeight: 1,
      })
    }
  ...
  private getNearestLocation(locs: { time: number[], lon: number[], lat: number[] }, target: number, opts = { tolerance: 1 / 24 / 4 }) {
    ...
    if (typeof(locs)=='object' && Array.isArray(locs.time) && Array.isArray(locs.lon) && Array.isArray(locs.lat)) {
