...
export class CtvreportComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    ...
    if (
      !this.dateData ||
      !this.dateData.general ||
      this.dateData.mmsi !== this.vesselObject.mmsi
    ) {
        ...
        .subscribe((_) => {
          this.getVideoRequests(this.vesselObject).subscribe((__) => {
            for (let i = 0; i < this.turbineTransfers.length; i++) {
              this.turbineTransfers[i].video_requested =
                this.matchVideoRequestWithTransfer(this.turbineTransfers[i]);
            }
  ...
  private getEngineStats() {
    return this.newService
      .getEnginedata(this.vesselObject.mmsi, this.vesselObject.date)
      .pipe(
        map((data) => {
          if (data.length > 0) {
            data[0]["fuelOther"] =
              data[0].fuelUsedTotalM3 -
              data[0].fuelUsedDepartM3 -
              data[0].fuelUsedReturnM3 -
              data[0].fuelUsedTransferM3;
            return data[0];
          } else {
            return {
              c02TotalKg: 0,
  ...
  private getTransfersForVessel() {
    return this.newService
      .getTransfersForVessel(this.vesselObject.mmsi, this.vesselObject.date)
      .pipe(
        map((transfers) => {
  ...
  private getGeneralStats(): Observable<any[]> {
    ...
    return this.newService.getGeneral(this.vesselObject).pipe(
      map((general) => {
        if (general && general.data && general.data.length > 0) {
          ...
          if (_general.DPRstats && typeof _general.DPRstats === "object") {
            ...
            dpr.AvgSpeedOutbound = this.switchUnit(
              dpr.AvgSpeedOutbound,
              "knots",
              this.settings.unit_speed
            );
            dpr.AvgSpeedInbound = this.switchUnit(
              dpr.AvgSpeedInbound,
              "knots",
              this.settings.unit_speed
            );
            dpr.AvgSpeedOutboundUnrestricted = this.switchUnit(
              dpr.AvgSpeedOutboundUnrestricted,
              "knots",
              this.settings.unit_speed
            );
            dpr.AvgSpeedInboundUnrestricted = this.switchUnit(
              dpr.AvgSpeedInboundUnrestricted,
              "knots",
              this.settings.unit_speed
            );
            dpr.sailedDistance = this.switchUnit(
              dpr.sailedDistance,
              "NM",
          ...
          if (_general.inputStats) {
            this.generalInputStats.fuelConsumption =
              _general.inputStats.fuelConsumption;
            this.generalInputStats.observations =
              _general.inputStats.observations;
            this.generalInputStats.landedGarbage =
              _general.inputStats.landedGarbage;
            ...
            this.generalInputStats.toolboxConducted =
              this.removeNansFromArray(_general.inputStats.toolboxConducted) ||
              [];
            this.generalInputStats.drillsConducted =
              this.removeNansFromArray(_general.inputStats.drillsConducted) ||
              [];
            this.generalInputStats.passengers = _general.inputStats.passengers;
            this.generalInputStats.customInput =
              _general.inputStats.customInput;
          }
          this.hasData = true;
        } else {
          this.showMap = false;
    ...
    if (this.dateData.transfer && this.dateData.general) {
      ...
      this.dateData.general.forEach((elt) => {
        formattedDate = this.dateTimeService.ymdStringToYMD(
          this.dateTimeService.matlabDatenumToYmdString(elt.date)
        );
        hasTransfers = this.dateData.transfer.reduce(
          (acc, val) => acc || +val === elt.date,
          false
        );
  ...
  private matchCommentsWithTransfers(_transfers) {
    for (let i = 0; i < _transfers.length; i++) {
      ...
      _transfers[i].commentChanged =
        _transfers[i].commentChanged || this.changedCommentObj;
      _transfers[i].formChanged = false;
      _transfers[i].video_requested = this.matchVideoRequestWithTransfer(
        _transfers[i]
      );
      for (let j = 0; j < this.commentsChanged.length; j++) {
        if (_transfers[i]._id === this.commentsChanged[j].idTransfer) {
  ...
  private createWeatherOverviewChart() {
    ...
    if (wavedata) {
      const timeStamps = wavedata.timeStamp.map((matlabTime) =>
        this.dateTimeService
          .matlabDatenumToMoment(matlabTime)
          .toISOString(false)
      );
  ...
  private matchVideoRequestWithTransfer(transfer): VideoRequestModel {
    ...
    if (vid) {
      ...
      if (vid.status !== "") {
        vid.text =
          vid.status[0].toUpperCase() + vid.status.substr(1).toLowerCase();
  ...
  public roundNumber(number, decimal = 10, addString = "") {
    return this.calculationService.roundNumber(
      number,
      (decimal = decimal),
      (addString = addString)
    );
  ...
  public getMatlabDateToJSTimeDifference(serialEnd, serialBegin) {
    return this.dateTimeService.getMatlabDatenumDifferenceString(
      serialEnd,
      serialBegin
    );
  }
